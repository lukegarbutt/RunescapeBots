...
def find_ge_limit(item_name, wiki_source):
	...
	print(search_word)
	split_text = re.split(r'{0}'.format(search_word), wiki_source, maxsplit=1, flags=0) # splits the source code into 2 at the position of the word we are looking for, this means that the ge limit of the item is the first number in the second element of the split_text variable
	print(len(split_text))
	result = re.search(r'(\d*)(,?)(\d*)', split_text[1]) # tries to locate the first number in the second entry of our split_text list. This number is in the format (any number of digits) + (,) + (any number of digits). This allows us to find numbers such as 25,000
